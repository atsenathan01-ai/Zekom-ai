...
// Priority list of valid candidate models to cascade through
...
// Router to handle all API endpoints reliably
...
apiRouter.post("/ai/create-project", async (req, res) => {
  try {
    const { idea } = req.body;
...
apiRouter.post("/ai/pitch", async (req, res) => {
  try {
...
apiRouter.post("/ai/secret-experiment", async (req, res) => {
  try {
...
apiRouter.get("/ai/challenges", async (_req, res) => {
  try {
    const response = await generateWithRetry(async (modelName, ai) => {
      return await ai.models.generateContent({
        model: modelName,
...
// Dual mounting: matches both /api/* (standard) and /* (if Vercel rewrite strips prefix)
